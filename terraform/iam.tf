...
resource "aws_iam_role_policy" "node_dynamodb_s3_policy" {
  name_prefix = "${var.project_name}-node-dynamodb-s3-"
...
resource "aws_iam_role" "argocd_role" {
  ...
  name_prefix = "${var.project_name}-argocd-"
...
resource "aws_iam_role_policy" "argocd_policy" {
  count       = var.enable_argocd ? 1 : 0
  name_prefix = "${var.project_name}-argocd-"
  role        = aws_iam_role.argocd_role[0].id
